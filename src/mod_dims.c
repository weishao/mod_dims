...
dims_handler(request_rec *r)
{
    ...
    if(strcmp(r->handler, "dims-local") == 0 &&
       strstr(r->canonical_filename, "__")) {
        /*
	 * Handle local filesystem images w/DIMS scale hint in file names.
         * The file name is in the format of filename__size.ext where size
	 * can be 'widthxheight', 'width', or 'xheight'. Both 'x' and 'X'
	 * can be used in size.
	 */

        /* Convert filename__size.ext back to filename.ext */
        char *scale_ptr = strstr(r->canonical_filename, "__");
	char *ext_ptr = strstr(scale_ptr, ".");
	d->filename = apr_pcalloc(r->pool, strlen(r->canonical_filename) + 1);
	strncpy(d->filename, r->canonical_filename, scale_ptr - r->canonical_filename);
	strcat(d->filename, ext_ptr);

	/* Construct commands */
	d->unparsed_commands = apr_pcalloc(r->pool, 32);
	strcat(d->unparsed_commands, "/AL/resize/");
	strncat(d->unparsed_commands, scale_ptr + 2, (ext_ptr - scale_ptr - 2));

	/* Change APLOG_DEBUG to APLOG_NOTICE to see log on stderr */
	ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, d->r, 
		      "Resize Handler: filename=%s, commands=%s", d->filename, d->unparsed_commands);

	apr_status_t status = dims_handle_request(d);

	/* Clean up */
	free(d->filename);
	free(d->unparsed_commands);
        d->filename = r->canonical_filename;
        d->unparsed_commands = r->path_info;

	return status;
    } else if(strcmp(r->handler, "dims-local") == 0 &&
